...
class SpaceDataAdmin(admin.ModelAdmin):
	...
	list_display = get_standard_display_list(SpaceData, additional_list=additional_list, excluding_list=["building"])
	list_display_links = ('S_ID', 'S_Name')
	list_filter = get_standard_display_list(SpaceData) + additional_list_filter
	inlines = [StructureInline, SupplySystemInline, ExhaustSystemInline, FancoilSystemInline, HeatSystemInline]
	actions = ['add_system']

	# class Media:
	# 	js = ['admin/js/datatables.js','admin/js/AdminFilterSelected.js']
	# 	css =['admin/css/datatables.css',]

	@form_action(SystemForm)
	@admin.action(description='Добавить систему')
	def add_system(self, request, queryset: List[SpaceData]):
		def _add_message(space: SpaceData, system: SystemData):
			return messages.success(request, f"обновлены следующие помещения {space.S_ID} "
			                                 f"добавлена система {system.system_type} имя {system.system_name}")

		def create_abstract_system(system: SystemData, _space: SpaceData, _defaults: dict[str, str],
		                           _create_defaults: dict[str, str]):
			system, created = system.objects.update_or_create(space_data=_space, create_defaults=_create_defaults,
			                                                  defaults=_defaults)
		...
		create_defaults = {val: form.cleaned_data[val] for val in
		                   get_standard_display_list(SystemData, excluding_list=['space_data'])}
