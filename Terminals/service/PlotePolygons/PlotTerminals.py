...
class PlotTerminalsAndSpaces:
	...
	def plot_terminals(self, ax):
		...
		for index, row in self.terminal_df.iterrows():
			PlotTerminalsAndSpaces.plot_scatters(ax, row["points"], float(row["dimension1"]), row["color"], row["geometry"])
			PlotTerminalsAndSpaces.add_text_to_df_terminals_points_column(ax, row.points, row[system_name])
